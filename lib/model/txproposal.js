 ...
 * getActors
 *
 * @return {String[]} zelerIds that performed actions in this proposal (accept / reject)
 */
TxProposal.prototype.getActors = function() {
  return _.pluck(this.actions, 'zelerId');
};
 ...
 * getApprovers
 *
 * @return {String[]} zelerIds that approved the tx proposal (accept)
 */
 ...
 * getActionBy
 *
 * @param {String} zelerId
 * @return {Object} type / createdOn
 */
TxProposal.prototype.getActionBy = function(zelerId) {
  return _.find(this.actions, {
    zelerId: zelerId
  });
};

TxProposal.prototype.addAction = function(zelerId, type, comment, signatures, xpub) {
...
TxProposal.prototype.sign = function(zelerId, signatures, xpub) {
  try {
    // Tests signatures are OK
    var tx = this.getBitcoreTx();
    this._addSignaturesToBitcoreTx(tx, signatures, xpub);

    this.addAction(zelerId, 'accept', null, signatures, xpub);
...
TxProposal.prototype.reject = function(zelerId, reason) {
  this.addAction(zelerId, 'reject', reason);
};
