...
Wallet.fromObj = function(obj) {
  ...
  x.zelers = _.map(obj.zelers, function(zeler) {
    return Copayer.fromObj(zeler);
  });
  x.pubKey = obj.pubKey;
...
/**
 * Get the maximum allowed number of required zelers.
 * This is a limit imposed by the maximum allowed size of the scriptSig.
 * @param {number} totalCopayers - the total number of zelers
 * @return {number}
 */
...
Wallet.prototype._updatePublicKeyRing = function() {
  this.publicKeyRing = _.map(this.zelers, function(zeler) {
    return _.pick(zeler, ['xPubKey', 'requestPubKey']);
  });
};

Wallet.prototype.addCopayer = function(zeler) {
  $.checkState(zeler.coin == this.coin);

  this.zelers.push(zeler);
  if (this.zelers.length < this.n) return;

  this.status = 'complete';
  this._updatePublicKeyRing();
};

Wallet.prototype.addCopayerRequestKey = function(zelerId, requestPubKey, signature, restrictions, name) {
  $.checkState(this.zelers.length == this.n);

  var c = this.getCopayer(zelerId);
...
Wallet.prototype.getCopayer = function(zelerId) {
  return _.find(this.zelers, {
    id: zelerId
  });
};
