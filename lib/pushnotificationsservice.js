      ...
      async.waterfall([
        ...
        function(contents, next) {
          async.map(recipientsList, function(recipient, next) {
            ...
            self.storage.fetchPushNotificationSubs(recipient.zelerId, function(err, subs) {
              ...
              var notifications = _.map(subs, function(sub) {
                return {
                  ...
                  data: {
                    ...
                    zelerId: sjcl.codec.hex.fromBits(sjcl.hash.sha256.hash(recipient.zelerId))
                  },
                };
              });
...
PushNotificationsService.prototype._getRecipientsList = function(notification, notificationType, cb) {
  ...
  self.storage.fetchWallet(notification.walletId, function(err, wallet) {
    ...
    self.storage.fetchPreferences(notification.walletId, null, function(err, preferences) {
      ...
      recipientPreferences = _.indexBy(recipientPreferences, 'zelerId');

      var recipientsList = _.compact(_.map(wallet.zelers, function(zeler) {
        if ((zeler.id == notification.creatorId && notificationType.notifyCreatorOnly) ||
          (zeler.id != notification.creatorId && !notificationType.notifyCreatorOnly)) {
          var p = recipientPreferences[zeler.id] || {};
          return {
            zelerId: zeler.id,
...
PushNotificationsService.prototype._getDataForTemplate = function(notification, recipient, cb) {
  ...
  self.storage.fetchWallet(notification.walletId, function(err, wallet) {
    ...
    var zeler = _.find(wallet.zelers, {
      id: notification.creatorId
    });

    if (zeler) {
      data.zelerId = zeler.id;
      data.zelerName = zeler.name;
    }

    if (notification.type == 'TxProposalFinallyRejected' && data.rejectedBy) {
      var rejectors = _.map(data.rejectedBy, function(zelerId) {
        return _.find(wallet.zelers, {
          id: zelerId
        }).name
      });
