...
/**
 * Gets an instance of the server after authenticating the zeler.
 * @param {Object} opts
 * @param {string} opts.zelerId - The zeler id making the request.
 * @param {string} opts.message - (Optional) The contents of the request to be signed. Only needed if no session token is provided.
 * @param {string} opts.signature - (Optional) Signature of message to be verified using one of the zeler's requestPubKeys. Only needed if no session token is provided.
 * @param {string} opts.session - (Optional) A valid session token previously obtained using the #login method
 * @param {string} opts.clientVersion - A string that identifies the client issuing the request
 * @param {string} [opts.walletId] - The wallet id to use as current wallet for this request (only when zeler is support staff).
 */
WalletService.getInstanceWithAuth = function(opts, cb) {
  function withSignature(cb) {
    if (!checkRequired(opts, ['zelerId', 'message', 'signature'], cb)) return;
    ...
    server.storage.fetchCopayerLookup(opts.zelerId, function(err, zeler) {
      if (err) return cb(err);
      if (!zeler) return cb(new ClientError(Errors.codes.NOT_AUTHORIZED, 'Copayer not found'));

      if (!zeler.isSupportStaff) {
        var isValid = !!server._getSigningKey(opts.message, opts.signature, zeler.requestPubKeys);
        if (!isValid)
          return cb(new ClientError(Errors.codes.NOT_AUTHORIZED, 'Invalid signature'));
        server.walletId = zeler.walletId;
      } else {
        server.walletId = opts.walletId || zeler.walletId;
        server.zelerIsSupportStaff = true;
      }

      server.zelerId = opts.zelerId;
      return cb(null, server);
    });
  };

  function withSession(cb) {
    if (!checkRequired(opts, ['zelerId', 'session'], cb)) return;
    ...
    server.storage.getSession(opts.zelerId, function(err, s) {
      if (err) return cb(err);
...
WalletService.prototype.login = function(opts, cb) {
  ...
  async.series([
    ...
    function(next) {
      self.storage.getSession(self.zelerId, function(err, s) {
    ...
    if (err) return cb(err);
    if (!session) return cb(new Error('Could not get current session for this zeler'));
...
WalletService.prototype.logout = function(opts, cb) {
  ...
  self.storage.removeSession(self.zelerId, cb);
};
 ...
 * @param {number} opts.m - Required zelers.
 * @param {number} opts.n - Total zelers.
 * @param {string} opts.pubKey - Public key to verify zelers joining have access to the wallet secret.
 * @param {string} opts.singleAddress[=false] - The wallet will only ever have one address.
 * @param {string} opts.coin[='zel'] - The coin for this wallet (btc, bch).
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  if (!Wallet.verifyCopayerLimits(opts.m, opts.n))
    return cb(new ClientError('Invalid combination of required zelers / total zelers'));
 ...
 * @param {Object} opts.includeExtendedInfo - Include PKR info & address managers for wallet & zelers
  ...
  async.parallel([
    ...
    function(next) {
      self.getWallet({}, function(err, wallet) {
        ...
        var zelerExtendedKeys = ['xPubKey', 'requestPubKey', 'signature', 'addressManager', 'customData'];

        wallet.zelers = _.map(wallet.zelers, function(zeler) {
          if (zeler.id == self.zelerId) return zeler;
          return _.omit(zeler, 'customData');
        });
        if (!opts.includeExtendedInfo) {
          wallet = _.omit(wallet, walletExtendedKeys);
          wallet.zelers = _.map(wallet.zelers, function(zeler) {
            return _.omit(zeler, zelerExtendedKeys);
          });
        }
 ...
 * @param {Object} data
 * @param {Object} opts
 * @param {Boolean} opts.isGlobal - If true, the notification is not issued on behalf of any particular zeler (defaults to false)
 */
WalletService.prototype._notify = function(type, data, opts, cb) {
  ...
  var zelerId = self.zelerId || data.zelerId;
  ...
  var notification = Model.Notification.create({
    ...
    creatorId: opts.isGlobal ? null : zelerId,
    walletId: walletId,
  });
...
WalletService.prototype._addCopayerToWallet = function(wallet, opts, cb) {
  ...
  var zeler = Model.Copayer.create({
    coin: wallet.coin,
    name: opts.name,
    zelerIndex: wallet.zelers.length,
    xPubKey: opts.xPubKey,
    requestPubKey: opts.requestPubKey,
    signature: opts.zelerSignature,
    customData: opts.customData,
    derivationStrategy: wallet.derivationStrategy,
  });

  self.storage.fetchCopayerLookup(zeler.id, function(err, res) {
    if (err) return cb(err);
...
WalletService.prototype._addKeyToCopayer = function(wallet, zeler, opts, cb) {
  var self = this;
  wallet.addCopayerRequestKey(zeler.zelerId, opts.requestPubKey, opts.signature, opts.restrictions, opts.name);
  self.storage.storeWalletAndUpdateCopayersLookup(wallet, function(err) {
    if (err) return cb(err);
...
/**
 * Adds access to a given zeler
 *
 * @param {Object} opts
 * @param {string} opts.zelerId - The zeler id
 * @param {string} opts.requestPubKey - Public Key used to check requests from this zeler.
 * @param {string} opts.zelerSignature - S(requestPubKey). Used by other zelers to verify the that the zeler is himself (signed with REQUEST_KEY_AUTH)
...
WalletService.prototype.addAccess = function(opts, cb) {
  ...
  if (!checkRequired(opts, ['zelerId', 'requestPubKey', 'signature'], cb)) return;

  self.storage.fetchCopayerLookup(opts.zelerId, function(err, zeler) {
    if (err) return cb(err);
    if (!zeler) return cb(Errors.NOT_AUTHORIZED);
    self.storage.fetchWallet(zeler.walletId, function(err, wallet) {
      if (err) return cb(err);
      if (!wallet) return cb(Errors.NOT_AUTHORIZED);

      var xPubKey = _.find(wallet.zelers, {
        id: opts.zelerId
      }).xPubKey;
      ...
      if (zeler.requestPubKeys.length > Defaults.MAX_KEYS)
        return cb(Errors.TOO_MANY_KEYS);

      self._addKeyToCopayer(wallet, zeler, opts, cb);
    });
  });
};
 ...
 * @param {string} opts.coin[='zel'] - The expected coin for this wallet (btc, bch).
 * @param {string} opts.name - The zeler name.
 * @param {string} opts.xPubKey - Extended Public Key for this zeler.
 * @param {string} opts.requestPubKey - Public Key used to check requests from this zeler.
 * @param {string} opts.zelerSignature - S(name|xPubKey|requestPubKey). Used by other zelers to verify that the zeler joining knows the wallet secret.
 * @param {string} opts.customData - (optional) Custom data for this zeler.
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  if (!checkRequired(opts, ['walletId', 'name', 'xPubKey', 'requestPubKey', 'zelerSignature'], cb)) return;

  if (_.isEmpty(opts.name))
    return cb(new ClientError('Invalid zeler name'));
  ...
  self._runLocked(cb, function(cb) {
    self.storage.fetchWallet(opts.walletId, function(err, wallet) {
      ...
      if (!self._verifySignature(hash, opts.zelerSignature, wallet.pubKey)) {
        return cb(new ClientError());
      }

      if (_.find(wallet.zelers, {
        xPubKey: opts.xPubKey
      })) return cb(Errors.COPAYER_IN_WALLET);

      if (wallet.zelers.length == wallet.n) return cb(Errors.WALLET_FULL);
...
/**
 * Save zeler preferences for the current wallet/zeler pair.
 * @param {Object} opts
...
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.contains(['ZEL', 'bit'], value.toLowerCase());
    },
  }];
  ...
  self._runLocked(cb, function(cb) {
    self.storage.fetchPreferences(self.walletId, self.zelerId, function(err, oldPref) {
      if (err) return cb(err);
...
/**
 * Retrieves a preferences for the current wallet/zeler pair.
 * @param {Object} opts
...
WalletService.prototype.getPreferences = function(opts, cb) {
  ...
  self.storage.fetchPreferences(self.walletId, self.zelerId, function(err, preferences) {
    if (err) return cb(err);
...
/**
 * Verifies that a given message was actually sent by an authorized zeler.
 * @param {Object} opts
...
WalletService.prototype.verifyMessageSignature = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    var zeler = wallet.getCopayer(self.zelerId);

    var isValid = !!self._getSigningKey(opts.message, opts.signature, zeler.requestPubKeys);
...
WalletService.prototype._canCreateTx = function(cb) {
  var self = this;
  self.storage.fetchLastTxs(self.walletId, self.zelerId, 5 + Defaults.BACKOFF_OFFSET, function(err, txs) {
    if (err) return cb(err);
...
/**
 * Publish an already created tx proposal so inputs are locked and other zelers in the wallet can see it.
 * @param {Object} opts
 * @param {string} opts.txProposalId - The tx id.
 * @param {string} opts.proposalSignature - S(raw tx). Used by other zelers to verify the proposal.
 */
  ...
  self._runLocked(cb, function(cb) {
    self.getWallet({}, function(err, wallet) {
      ...
      self.storage.fetchTx(self.walletId, opts.txProposalId, function(err, txp) {
        ...
        var signingKey = self._getSigningKey(raw, opts.proposalSignature, zeler.requestPubKeys);
        if (!signingKey) {
          return cb(new ClientError('Invalid proposal signature'));
        }

        // Save signature info for other zelers to check
  ...
  self._runLocked(cb, function(cb) {
    self.storage.fetchTxNote(self.walletId, opts.txid, function(err, note) {
      ...
      if (!note) {
        ...
        note.edit(opts.body, self.zelerId);
      }
...
WalletService.prototype.getRemainingDeleteLockTime = function(txp) {
  ...
  if (txp.creatorId !== this.zelerId)
    return lockTimeRemaining;

  // has other approvers? need to wait
  var approvers = txp.getApprovers();
  if (approvers.length > 1 || (approvers.length == 1 && approvers[0] !== this.zelerId))
    return lockTimeRemaining;
 ...
 * @param {string} opts.signatures - The signatures of the inputs of this tx for this zeler (in apperance order)
 */
WalletService.prototype.signTx = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self.getTx({
      ...
      try {
        if (!txp.sign(self.zelerId, opts.signatures, zeler.xPubKey)) {
          log.warn('Error signing transaction (BAD_SIGNATURES)');
          log.warn('Wallet id:', self.walletId);
          log.warn('Copayer id:', self.zelerId);
 ...
 * @param {Object} opts
 * @param {string} opts.txProposalId - The identifier of the transaction.
 * @param {string} [opts.reason] - A message to other zelers explaining the rejection.
 */
WalletService.prototype.rejectTx = function(opts, cb) {
  ...
  self.getTx({
    ...
    txp.reject(self.zelerId, opts.reason);
    ...
    return _.map(txs, function(tx) {
      ...
      if (proposal) {
        ...
        newTx.actions = _.map(proposal.actions, function(action) {
          return _.pick(action, ['createdOn', 'type', 'zelerId', 'zelerName', 'comment']);
        });
        ...
        self.storage.storeWallet(wallet, function(err) {
          ...
          _.each([false, true], function(isChange) {
            ...
            if (opts.includeCopayerBranches) {
              _.each(wallet.zelers, function(zeler) {
                if (zeler.addressManager) {
                  derivators.push({
                    derive: _.bind(zeler.createAddress, zeler, wallet, isChange),
                    rewind: _.bind(zeler.addressManager.rewindIndex, zeler.addressManager, isChange),
                  });
                }
              });
            }
...
/**
 * Subscribe this zeler to the Push Notifications service using the specified token.
 * @param {Object} opts
...
/**
 * Unsubscribe this zeler to the Push Notifications service using the specified token.
 * @param {Object} opts
 * @param {string} opts.token - The token representing the app/device.
 */
WalletService.prototype.pushNotificationsUnsubscribe = function(opts, cb) {
  ...
  self.storage.removePushNotificationSub(self.zelerId, opts.token, cb);
};

/**
 * Subscribe this zeler to the specified tx to get a notification when the tx confirms.
 * @param {Object} opts
...
WalletService.prototype.txConfirmationUnsubscribe = function(opts, cb) {
  ...
  self.storage.removeTxConfirmationSub(self.zelerId, opts.txid, cb);
};
