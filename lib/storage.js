...
Storage.prototype.storeWalletAndUpdateCopayersLookup = function(wallet, cb) {
  ...
  var zelerLookups = _.map(wallet.zelers, function(zeler) {
    $.checkState(zeler.requestPubKeys);
    return {
      zelerId: zeler.id,
      walletId: wallet.id,
      requestPubKeys: zeler.requestPubKeys,
    };
  });
  ...
  this.db.collection(collections.COPAYERS_LOOKUP).remove({
    ...
    if (err) return cb(err);
    self.db.collection(collections.COPAYERS_LOOKUP).insert(zelerLookups, {
      w: 1
    }, function(err) {
      if (err) return cb(err);
...
Storage.prototype.fetchCopayerLookup = function(zelerId, cb) {
  ...
  self.fetchWallet(walletId, function(err, wallet) {
    ...
    _.each([].concat(txs), function(tx) {
      ...
      _.each(tx.actions, function(action) {
        action.zelerName = wallet.getCopayer(action.zelerId).name;
      });
...
Storage.prototype.fetchPreferences = function(walletId, zelerId, cb) {
...
Storage.prototype.getSession = function(zelerId, cb) {
  var self = this;
...
Storage.prototype.fetchPushNotificationSubs = function(zelerId, cb) {
...
Storage.prototype.storePushNotificationSub = function(pushNotificationSub, cb) {
  this.db.collection(collections.PUSH_NOTIFICATION_SUBS).update({
    zelerId: pushNotificationSub.zelerId,
...
Storage.prototype.removePushNotificationSub = function(zelerId, token, cb) {
...
Storage.prototype.fetchActiveTxConfirmationSubs = function(zelerId, cb) {
  var filter = {
    isActive: true
  };
  if (zelerId) filter.zelerId = zelerId;
...
Storage.prototype.storeTxConfirmationSub = function(txConfirmationSub, cb) {
  this.db.collection(collections.TX_CONFIRMATION_SUBS).update({
    zelerId: txConfirmationSub.zelerId,
...
Storage.prototype.removeTxConfirmationSub = function(zelerId, txid, cb) {
