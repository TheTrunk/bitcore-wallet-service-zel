...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  if (opts.disableLogs) {
    ...
    morgan.token('zelerId', function getId(req) {
      return req.zelerId
    });

    var logFormat = ':remote-addr :date[iso] ":method :url" :status :res[content-length] :response-time ":user-agent" :walletId :zelerId';
    var logOpts = {
  ...
  function getServerWithAuth(req, res, opts, cb) {
    ...
    WalletService.getInstanceWithAuth(auth, function(err, server) {
      ...
      if (opts.onlySupportStaff && !server.zelerIsSupportStaff) {
        return returnError(new WalletService.ClientError({
          code: 'NOT_AUTHORIZED'
        }), res, req);
      }
  ...
  router.put('/v1/zelers/:id/', function(req, res) {
    req.body.zelerId = req.params['id'];
    var server;
    try {
      server = getServer(req, res);
    } catch (ex) {
  ...
  // DEPRECATED
  router.post('/v1/wallets/:id/zelers/', function(req, res) {
    logDeprecated(req);
  ...
  router.post('/v2/wallets/:id/zelers/', function(req, res) {
    req.body.walletId = req.params['id'];
    var server;
    try {
      server = getServer(req, res);
