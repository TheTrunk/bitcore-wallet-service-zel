...
See [Bitcore-wallet-client](https://github.com/zelcash/bitcore-wallet-client-zel) for the *official* client library that communicates to BWS and verifies its response. Also check [Bitcore-wallet](https://github.com/bitpay/bitcore-wallet) for a simple CLI wallet implementation that relies on BWS.
...
# Getting Started
```
 git clone https://github.com/zelcash/bitcore-wallet-service-zel.git
...
BWS supports SSL and Clustering. For a detailed guide on installing BWS with extra features see [Installing BWS](https://github.com/zelcash/bitcore-wallet-service-zel/blob/master/installation.md).
...
See [Bitcore Wallet Client](https://github.com/zelcash/bitcore-wallet-client-zel/blob/master/lib/api.js#L73) for implementation details.
...
Returns:
 * Wallet object. (see [fields on the source code](https://github.com/zelcash/bitcore-wallet-service-zel/blob/master/lib/model/wallet.js)).
...
Returns:
 * List of pending TX Proposals. (see [fields on the source code](https://github.com/zelcash/bitcore-wallet-service-zel/blob/master/lib/model/txproposal.js))

`/v1/addresses/`: Get Wallet's main addresses (does not include change addresses)

Returns:
 * List of Addresses object: (https://github.com/zelcash/bitcore-wallet-service-zel/blob/master/lib/model/address.js)).  This call is mainly provided so the client check this addresses for incoming transactions (using a service like [Insight](https://insight.is)
...
Returns:
 * TX Proposal object. (see [fields on the source code](https://github.com/zelcash/bitcore-wallet-service-zel/blob/master/lib/model/txproposal.js)). `.id` is probably needed in this case.


`/v1/addresses/`: Request a new main address from wallet

Returns:
 * Address object: (https://github.com/zelcash/bitcore-wallet-service-zel/blob/master/lib/model/address.js)). Note that `path` is returned so client can derive the address independently and check server's response.
...
Returns:
 * TX Proposal object. (see [fields on the source code](https://github.com/zelcash/bitcore-wallet-service-zel/blob/master/lib/model/txproposal.js)). `.status` is probably needed in this case.
