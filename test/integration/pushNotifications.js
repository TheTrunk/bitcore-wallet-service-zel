...
describe('Push notifications', function() {
  ...
  describe('Single wallet', function() {
    ...
    it('should build each notifications using preferences of the zelers', function(done) {
      server.savePreferences({
        language: 'en',
        unit: 'bit',
      }, function(err) {
    ...
    it('should notify zelers when payment is received', function(done) {
      server.createAddress({}, function(err, address) {
        should.not.exist(err);
      ...
      helpers.stubUtxos(server, wallet, 1, function() {
        ...
        async.waterfall([
          ...
          function(t, next) {
            ...
            async.eachSeries(_.range(1, 3), function(i, next) {
              var zeler = TestData.zelers[i];
              helpers.getAuthServer(zeler.id44btc, function(s) {
                server = s;
          ...
          setTimeout(function() {
            ...
            sjcl.codec.hex.fromBits(sjcl.hash.sha256.hash(server.zelerId)).should.not.equal(args[0].body.data.zelerId);
            sjcl.codec.hex.fromBits(sjcl.hash.sha256.hash(server.zelerId)).should.not.equal(args[1].body.data.zelerId);
            done();
          }, 100);
        });
      });
  ...
  describe('joinWallet', function() {
    ...
    it('should notify zelers when a new zeler just joined into your wallet except the one who joined', function(done) {
      async.eachSeries(_.range(3), function(i, next) {
        var zelerOpts = helpers.getSignedCopayerOpts({
          walletId: walletId,
          name: 'zeler ' + (i + 1),
          xPubKey: TestData.zelers[i].xPubKey_44H_0H_0H,
          requestPubKey: TestData.zelers[i].pubKey_1H_0,
          customData: 'custom data ' + (i + 1),
        });

        server.joinWallet(zelerOpts, function(err, res) {
          if (err) return next(err);

          helpers.getAuthServer(res.zelerId, function(server) {
            server.pushNotificationsSubscribe({
              token: 'token:' + zelerOpts.name,
              packageName: 'com.wallet',
              platform: 'Android',
            }, next);
          });
        ...
        setTimeout(function() {
          ...
          var args = _.filter(_.map(calls, function(call) {
            ...
            return arg.body.notification.title == 'New zeler';
          });

          server.getWallet(null, function(err, wallet) {
            /*
              First call - zeler2 joined
              zeler2 should notify to zeler1
              zeler2 should NOT be notifyed
            */
            var hashedCopayerIds = _.map(wallet.zelers, function(zeler) {
              return sjcl.codec.hex.fromBits(sjcl.hash.sha256.hash(zeler.id));
            });
            hashedCopayerIds[0].should.equal((args[0].body.data.zelerId));
            hashedCopayerIds[1].should.not.equal((args[0].body.data.zelerId));

            /*
              Second call - zeler3 joined
              zeler3 should notify to zeler1
            */
            hashedCopayerIds[0].should.equal((args[1].body.data.zelerId));

            /*
              Third call - zeler3 joined
              zeler3 should notify to zeler2
            */
            hashedCopayerIds[1].should.equal((args[2].body.data.zelerId));

            // zeler3 should NOT notify any other zeler
            hashedCopayerIds[2].should.not.equal((args[1].body.data.zelerId));
            hashedCopayerIds[2].should.not.equal((args[2].body.data.zelerId));
            done();
          });
        }, 100);
      });
