...
describe('Storage', function() {
  ...
  describe('Copayer lookup', function() {
    it('should correctly store and fetch zeler lookup', function(done) {
      var wallet = Model.Wallet.create({
        id: '123',
        name: 'my wallet',
        m: 2,
      ...
      storage.storeWalletAndUpdateCopayersLookup(wallet, function(err) {
        should.not.exist(err);
        storage.fetchCopayerLookup(wallet.zelers[1].id, function(err, lookup) {
          should.not.exist(err);
          should.exist(lookup);
    ...
    it('should not return error if zeler not found', function(done) {
  ...
  describe('Transaction proposals', function() {
    ...
    it('should fetch tx', function(done) {
      storage.fetchTx('123', proposals[0].id, function(err, tx) {
        ...
        tx.creatorName.should.equal('zeler 0');
        done();
      });
    });
